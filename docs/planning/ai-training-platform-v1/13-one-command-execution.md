...
& $trustedGpg --homedir $gpgHome --no-options --no-auto-key-retrieve --no-auto-check-trustdb --no-autostart --lock-never --batch --no-tty --with-colons --fingerprint --list-keys
```
