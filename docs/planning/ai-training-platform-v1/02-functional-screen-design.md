...
역할은 한 사용자가 복수 보유할 수 있다. 화면 노출과 API 권한은 동일한 정책 ID를 사용하며, 기관 범위는 ABAC 조건으로 추가 제한한다.
...
| PUB-004 | 인증 | 로그인·가입·재설정·약관 | 오류가 계정 존재 여부를 노출하지 않음 |
...
| TCH-015 | 초안 편집·검수 | 섹션 편집, 근거, 변경비교, 검수 요청 | 승인 전 워터마크, 내보내기 이력 기록 |
...
| ADM-012 | 사용자·권한 | 사용자, 기관, 역할, 동의, session | 고권한 변경에 재인증·감사 기록 |
...
- 1440 px: 최대 1280 px 콘텐츠, 좌측 navigation과 보조 panel 허용
- 768 px: 접이식 navigation, 2열을 1열로 전환
- 390 px: 하단 핵심 navigation, sticky primary action, 수평 overflow 금지
